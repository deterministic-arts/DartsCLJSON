...
stream-based pull parser as well as a push-style parser.
 ...
 - *Condition* `parse-error` 
   ...
   Class precedence list `parse-error`, `error`, `serious-condition`, `condition`
 ...
 - *Condition* `simple-parse-error` 
   ...
   Class precedence list `simple-parse-error`, `simple-condition`,  `parse-error`, `error`, `serious-condition`, `condition`

 - *Function* `read-value` _stream_ &rarr; _value_
 ...
 - *Function* `make-push-parser` _callback_ `&key` _byte-input_ &rarr; _parser_
   ...
   Note that the parser is greedy, i.e.,
       ...
       (let ((parser (make-push-parser #'print)))
         (funcall parser "12")
         (funcall parser "34")
         (funcall parser nil))
...
### Auxiliary Functions And Macros
 ...
 - *Function* `read-token` _stream_ &rarr; _value_
 - *Function* `read-value-from-string` _string_ `&optional` (_start_ 0) _end_ &rarr; _value_
...
### High-Level
 ...
 - *Function* `write-null`
 - *Function* `write-number` _value_
 - *Function* `write-boolean` _value_
 - *Function* `write-string` _value_ `&key` _start_ _end_
 - *Function* `write-key` _value_ `&key` _start_ _end_
 - *Macro* `writing-array` `&body` _body_
 - *Macro* `writing-object` `&body` _body_
 - *Macro* `with-output-context` `(` _stream_ `&key` _ascii-only_ _dense-output_ `)` `&body` _body_ 

### Low-Level 

 - *Function* `render-null` _stream_ &rarr; _nil_
 - *Function* `render-boolean` _value_ _stream_ &rarr; _value_
 - *Function* `render-number` _value_ _stream_ &rarr; _value_
 - *Function* `render-string` _value_ _stream_ `&key` _start_ _end_ _ascii-only_ &rarr; _value_
 - *Function* `render-array-start` _stream_ &rarr; _nil_
 - *Function* `render-array-end` _stream_ &rarr; _nil_
 - *Function* `render-object-start` _stream_ &rarr; _nil_
 - *Function* `render-object-end` _stream_ &rarr; _nil_
 - *Function* `render-key` _value_ _stream_ `&key` _start_ _end_ _ascii-only_ _dense-outut_ &rarr; _value_
...
macros are defined
 ...
 - *Macro* `if-bind` _pattern_ _form_ _then_ _else_ &rarr; _values_
 ...
 - *Macro* `when-bind` _pattern_ _form_ `&body` _body_ &rarr; _values_
   ...
   matching fails, the `when-bind` form returns `nil`.
   
 - *Macro* `match` _form_ `&body` _clauses_ &rarr; _values_
 ...
 - *Macro* `ematch` _form_ `&body` _clauses_ &rarr; _values_
...
Examples:
    ...
    (json:if-bind (:object identifier &rest properties) '(:object "name" "dummy" "identifier" 17)
